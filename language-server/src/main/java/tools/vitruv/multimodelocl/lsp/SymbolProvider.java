...
public class SymbolProvider {
  ...
  public List<Either<SymbolInformation, DocumentSymbol>> getSymbols(DocumentAnalysis analysis) {
    ...
    OCLParser.ContextDeclCSContext root = analysis.getTree();
    List<Either<SymbolInformation, DocumentSymbol>> result = new ArrayList<>();

    // Use the typed ANTLR accessor — avoids manually walking mixed child lists
    for (OCLParser.ClassifierContextCSContext ctx : root.classifierContextCS()) {
  ...
  private static DocumentSymbol buildContextSymbol(OCLParser.ClassifierContextCSContext ctx) {
    String name;
  ...
  private static DocumentSymbol buildInvSymbol(OCLParser.InvCSContext ctx) {
  ...
  private static Range nameSelectionRange(OCLParser.ClassifierContextCSContext ctx) {
    if (ctx.metamodel != null && ctx.className != null) {
      ...
      int stopChar  = Math.max(0, ctx.className.getCharPositionInLine()
                                 + ctx.className.getText().length());
      return new Range(new Position(startLine, startChar), new Position(stopLine, stopChar));
    }
