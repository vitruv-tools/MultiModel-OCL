 ...
 *
 * <p>When the user types {@code (} or {@code ,} inside an OCL# expression the language server
 * calls this provider. It scans backward from the cursor to find the innermost active call,
 * looks up the operation name in {@link OclOperationDocs}, and returns a {@link SignatureHelp}
 * with the correct active parameter highlighted.
 *
 * <p>Two parameter-counting strategies are used depending on the operation:
 * <ul>
 *   <li><b>Iterator operations</b> (signature contains {@code |}): the pipe character separates
 *       the iterator variable from the body expression.  Cursor before {@code |} → first param
 *       highlighted; cursor after {@code |} → second param highlighted.
 *   <li><b>Regular operations</b> (e.g. {@code substring(lower, upper)}): commas at depth 0
 *       between the opening {@code (} and the cursor determine the active parameter index.
 * </ul>
 */
final class SignatureHelpProvider {
  ...
  private static SignatureInformation buildSignature(OclOperationDocs.OperationDoc doc) {
    ...
    List<ParameterInformation> paramInfos = doc.params().stream()
        .map(p -> {
   ...
   * @param cursorAfterPipe {@code true} when a top-level {@code |} was found between {@code (}
   *                        and the cursor (iterator body region)
   */
