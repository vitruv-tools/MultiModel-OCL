...
final class InlayHintProvider {
  ...
  private static final class Walker {
    ...
    void visit(ParseTree node) {
      ...
      if (node instanceof OCLParser.VariableDeclarationContext ctx) {
        handleVarDecl(ctx);
      }

      // ── Iterator-variable type hints ───────────────────────────────────────
      if (node instanceof OCLParser.SelectOpContext ctx) {
        handleIteratorVars(ctx.iteratorVars, ctx.body);
      }
      if (node instanceof OCLParser.RejectOpContext ctx) {
        handleIteratorVars(ctx.iteratorVars, ctx.body);
      }
      if (node instanceof OCLParser.CollectOpContext ctx) {
        handleIteratorVars(ctx.iteratorVars, ctx.body);
      }
      if (node instanceof OCLParser.ForAllOpContext ctx) {
        handleIteratorVars(ctx.iteratorVars, ctx.body);
      }
      if (node instanceof OCLParser.ExistsOpContext ctx) {
        handleIteratorVars(ctx.iteratorVars, ctx.body);
      }

      // ── Property-navigation type hint (metaclass attributes) ───────────────
      if (node instanceof OCLParser.PropertyNavContext ctx) {
    ...
    private void handleVarDecl(OCLParser.VariableDeclarationContext ctx) {
    ...
    private void handleIteratorVars(
        OCLParser.IteratorVarListContext varList, OCLParser.ExpCSContext body) {
    ...
    private void handlePropertyNav(OCLParser.PropertyNavContext ctx) {
     ...
     */
    private Type resolveExpType(OCLParser.ExpCSContext ctx) {
    ...
    private Type findFirstUsageType(ParseTree node, String varName) {
      ...
      if (node instanceof OCLParser.VariableExpCSContext ctx) {
      ...
      if (node instanceof OCLParser.VariableContext ctx
