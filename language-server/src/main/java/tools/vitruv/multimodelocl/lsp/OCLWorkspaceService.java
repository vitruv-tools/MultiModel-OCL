...
/**
 * Workspace service that hot-reloads {@code .ecore} files whenever VS Code reports a
 * {@code workspace/didChangeWatchedFiles} event for them.
 *
 * <p>When the server starts it registers a glob watcher for {@code **}{@code /*.ecore} (see
 * {@link OCLLanguageServer#initialized}).  VS Code then sends {@code didChangeWatchedFiles}
 * notifications whenever any matching file is created, modified, or deleted.
 *
 * <p>On every such notification:
 * <ol>
 *   <li>The affected metamodels are reloaded (or removed) in {@link MetamodelWrapper}.
 *   <li>All currently-open OCL documents are re-analysed so that the updated type information
 *       is reflected immediately without requiring a dummy edit.
 * </ol>
 */
public class OCLWorkspaceService implements WorkspaceService {
  ...
  public OCLWorkspaceService(MetamodelWrapper wrapper,
      OCLTextDocumentService textDocumentService) {
