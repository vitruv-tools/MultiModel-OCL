...
public class CompletionProvider {
  ...
  private static final Pattern MM_COLON_COLON =
      Pattern.compile("(\\w+)::$");
  private static final Pattern MM_CLASS_DOT =
      Pattern.compile("(\\w+)::(\\w+)\\.$");
  private static final Pattern SELF_DOT =
      Pattern.compile("\\bself\\.$");
  private static final Pattern CONTEXT_DECL =
      Pattern.compile("\\bcontext\\s+(\\w+)::(\\w+)");

  // Type-position patterns: after "let x :" and after type-cast / type-test operations.
  private static final Pattern LET_TYPE_POS =
      Pattern.compile("\\blet\\s+\\w+\\s*:\\s*\\w*$");
  ...
  private static final List<String> COLLECTION_OPS = List.of(
      "select", "reject", "collect", "forAll", "exists",
      "includes", "excludes", "including", "excluding",
      "isEmpty", "notEmpty", "size", "flatten", "union",
      "append", "prepend", "sum", "max", "min", "avg",
      "first", "last", "reverse", "sortedBy",
  ...
  private static final List<String> TOP_LEVEL_KEYWORDS = List.of(
      "self", "allInstances", "let", "in", "if", "then", "else", "endif",
      "null", "true", "false", "not", "and", "or", "xor", "implies"
  );
  ...
  public List<CompletionItem> getCompletions(
    ...
    // -----------------------------------------------------------------------
    if (LET_TYPE_POS.matcher(textBefore).find()
        || TYPE_CAST_POS.matcher(textBefore).find()) {
      return typeItems();
    }
  ...
  private List<CompletionItem> featuresFor(EClass eClass) {
    ...
    for (EStructuralFeature feature : eClass.getEAllStructuralFeatures()) {
      ...
      String typeName =
          feature.getEType() != null ? feature.getEType().getName() : "?";
  ...
  /**
   * Returns type-completion items for positions where an OCL# type name is expected
   * (e.g. after {@code let x :} or inside {@code oclIsKindOf(}).
   *
  ...
  private List<CompletionItem> completionsFromType(
    ...
    ParseTree node = NodeFinder.findAt(analysis.getTree(), beforeDot.getLine(), beforeDot.getCharacter());
